...
def generate_specs_string(data):
    """
    Generate a unique string based on the specs of the data,
    including low_tensor and high_tensor.
    """
    specs = (f"{data['num_simulations']:.0f}sim_"
             f"{data['Npts']:.0e}np_{data['dt']:.0e}dt_"
             f"{data['oversampling']}os_{data['prerun']:.0e}pre_"
             f"{'_'.join(data['features'])}_"
             f"{torch.sum(data['low_tensor']).item():.0f}low_"
             f"{torch.sum(data['high_tensor']).item():.0f}high")
    return specs

def specs_hash(data):
    """
    Generate a hash of the specs for easy comparison.
    """
    specs_string = generate_specs_string(data)
    return hashlib.md5(specs_string.encode()).hexdigest()
...
def save_pickle_data(data, folder_path, merge=False):
    """
    ...
    dataset_type = 'posterior' if 'posterior' in data else 'dataset'
    
    # Generate specs hash
    specs_hash_value = specs_hash(data)
    
    # Construct the filename using the specs hash
    dataset_name = f"{dataset_type}_{specs_hash_value}.pickle"
    ...
    if merge and os.path.exists(dataset_path):
        ...
        if (data['num_simulations'] == existing_specs['num_simulations'] and 
            ...
            torch.equal(data['high_tensor'], existing_specs['high_tensor']) and
            data['features'] == existing_specs['features']):
            ...
            while os.path.exists(dataset_path):
                dataset_name = f"{dataset_type}_{specs_hash_value}_{counter}.pickle"
                dataset_path = os.path.join(folder_path, dataset_name)
                counter += 1
    else:
