...
def entropy(x, y, fx, fy, tlist, dt, D1):
    ...
    Fsx=(fx[1:]+fx[:-1])/2
    Fsy=(fy[1:]+fy[:-1])/2
    S=np.sum(Fsx*dx)/(D1[0]*(tlist[-1]-1))+np.sum(Fsy*dy)/(D1[1]*(tlist[-1]-1))
    return S
...
def simulator_sbi(pars, dt, oversampling, prerun, Npts, prefix = None, d=0, k=1): 
    ...
    npcombo = np.stack((Cxx[idx_corr],Cxy[idx_corr], Cyy[idx_corr], S_red_x[idx_corr], S_red_y[idx_corr]))
    return npcombo
    ...
    if features:
        for feature in features:
            if feature == "Cxx":
                ...
                npcombo[current_index:current_index + len(Cxy[idx_corr])] = Cxy[idx_corr]
                current_index += len(Cxy[idx_corr])
            elif feature == "Cyy":
